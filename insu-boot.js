...
(function () {
  ...
  try {
    ...
    /* Gated document preview. Triggers carry a document id, never a file
       URL — the full PDF is not linked from this page and is not in the
       public asset repo, so the gate is real rather than cosmetic. */
    (function () {
      ...
      if (!gate || !triggers.length) return;

      /* position:fixed, so it must not sit under a transformed ancestor —
         that becomes its containing block and traps it in the panel. */
      if (gate.parentNode !== document.body) document.body.appendChild(gate);

      var img = gate.querySelector('[data-docgate-img]');
      var codeEl = gate.querySelector('[data-docgate-code]');
      var titleEl = gate.querySelector('[data-docgate-title]');
      var form = gate.querySelector('[data-docgate-form]');
      var input = gate.querySelector('.docgate-input');
      var btn = gate.querySelector('.docgate-btn');
      var msg = gate.querySelector('[data-docgate-msg]');
      ...
      function open(id) {
        ...
        current = id;
        lastFocus = document.activeElement;
        if (img) { img.src = d.preview; img.alt = id + ' opening page'; }
      ...
      triggers.forEach(function (el) {
        el.addEventListener('click', function (e) {
          e.preventDefault();
          open(el.getAttribute('data-doc-id'));
        });
      });

      var closeBtn = gate.querySelector('[data-docgate-close]');
      ...
      if (form) {
        form.addEventListener('submit', function (e) {
          ...
          fetch(GATE_ENDPOINT, {
            ...
            /* The endpoint does not exist yet. Never leave a form that
               swallows a request silently — name a real address. */
