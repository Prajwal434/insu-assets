...
(function () {
  ...
  try {
    ...
    /* Document deck. Same interaction as the thread carousel above:
       drag the cards or pull the bead, elastic snap, pulse along the
       line. Kept as its own instance rather than generalised — the
       thread is wired to the pillar headings and this is not. */
    (function () {
      ...
      if (viewer) {
        /* position:fixed, so it must not sit under a transformed
           ancestor — that becomes its containing block and traps it
           inside the panel. */
      ...
      function closeDoc() {
        ...
        if (frame) frame.removeAttribute('src');   // stop the download, start clean next time
        if (lastFocus && lastFocus.focus) lastFocus.focus();
      }
      if (viewer) {
        var cb = viewer.querySelector('[data-pdfviewer-close]');
        if (cb) cb.addEventListener('click', closeDoc);
        viewer.addEventListener('click', function (e) { if (e.target === viewer) closeDoc(); });
        document.addEventListener('keydown', function (e) {
      ...
      /* ---- drag carousel ---- */
      var cards = [].slice.call(track.querySelectorAll('.deckcard'));
      if (!cards.length) return;
      var path = document.getElementById('deckPath');
      var pulse = document.getElementById('deckPulse');
      var bead = document.getElementById('deckBead');
      var line = document.getElementById('deckLine');
      var idx = 0, step = 0, maxShift = 0;
      ...
      cards.forEach(function (c, i) {
        c.addEventListener('click', function (e) {
          if (moved > 6) { e.preventDefault(); return; }   // a drag, not a click
          if (i !== idx) { goTo(i); return; }              // bring it forward first
          var tmp = document.createElement('textarea');
          tmp.innerHTML = c.getAttribute('data-title') || '';
          openDoc(c.getAttribute('data-pdf'), tmp.value);
        });
      });
      if (bead) {
        bead.addEventListener('keydown', function (e) {
