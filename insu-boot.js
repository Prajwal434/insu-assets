...
(function () {
  ...
  /* First-line consultant: a guided intake in place of a bare form.
     Asks what the problem is, what it costs, how soon it matters, takes
     it in their own words, then names the capabilities that match and
     offers to book time. Rule-based, and the panel says so. */
  (function () {
    var ENDPOINT = '/_functions/contactRequest';
    var INBOX = 'info@insudynamics.com';
    /* Point at Wix Bookings or Calendly once one exists. Empty means we
       offer times by email rather than linking nowhere. */
    var BOOKING_URL = '';
    ...
    var stage = pop.querySelector('[data-bda-stage]');
    var fill = pop.querySelector('[data-bda-fill]');
    var sub = pop.querySelector('[data-bda-sub]');
    if (!stage) return;

    var CAPS = {
      planning:  ['Oracle ASCP and Fusion Cloud SCM', 'Plan setup, sourcing rules and constraint modelling', 'Exception design so planners read what matters'],
      inventory: ['Demand variability and ABC/XYZ segmentation', 'Safety stock and reorder policy per segment', 'Slow and obsolete stock surfaced early'],
      forecast:  ['Statistical baselines with business override', 'Bias and MAPE tracked across horizons', 'Consensus planning across functions'],
      agents:    ['Agents answering in plain language over your own data', 'Traceable answers, every figure with its source', 'Exception triage ranked by consequence'],
      infra:     ['Predictive monitoring ahead of the breach', 'First-level remediation without paging anyone', 'Each incident captured as a rule'],
      data:      ['Pipelines from scattered operational systems', 'Reporting people actually act on', 'Decision-ready measures rather than dashboards'],
      discover:  ['A short discovery to find where the cost really sits', 'An honest answer on whether AI is the right tool', 'A costed recommendation before anyone builds']
    };

    var Q = {
      area: { q: 'What are you bringing us?', a: [
        { t: 'Something in supply chain planning', next: 'sc', tag: 'Supply chain' },
        { t: 'Something we could automate with AI', next: 'ai', tag: 'AI and automation' },
        { t: 'Data we cannot get decisions out of', next: 'cost', caps: ['data'], tag: 'Data and reporting' },
        { t: 'Not sure yet, but something is wrong', next: 'cost', caps: ['discover'], tag: 'Discovery' }
      ]},
      sc: { q: 'Where does it hurt most?', a: [
        { t: 'The plan is not trusted', next: 'cost', caps: ['planning'] },
        { t: 'Too much stock, and still stocking out', next: 'cost', caps: ['inventory'] },
        { t: 'The forecast is always wrong', next: 'cost', caps: ['forecast'] },
        { t: 'Oracle is live but nobody uses it', next: 'cost', caps: ['planning'] }
      ]},
      ai: { q: 'What would you point it at?', a: [
        { t: 'Questions people ask the same way every week', next: 'cost', caps: ['agents'] },
        { t: 'Infrastructure and incidents', next: 'cost', caps: ['infra'] },
        { t: 'Documents and manual data entry', next: 'cost', caps: ['data','agents'] },
        { t: 'Predicting something before it happens', next: 'cost', caps: ['forecast','agents'] }
      ]},
      cost: { q: 'What is it costing you today?', a: [
        { t: "People's time, every week", next: 'when' },
        { t: 'Money, in stock, expedites or overtime', next: 'when' },
        { t: 'Decisions made late or on guesswork', next: 'when' },
        { t: 'Hard to put a number on it yet', next: 'when' }
      ]},
      when: { q: 'How soon does this need to move?', a: [
        { t: 'It is urgent', next: null },
        { t: 'This quarter', next: null },
        { t: 'We are planning ahead', next: null }
      ]}
    };

    var key, caps, notes, tag, words, history, det;
    function reset() {
      key = 'area'; caps = []; notes = []; tag = ''; words = ''; history = [];
      det = { name: '', email: '', company: '' };
      if (sub) sub.textContent = 'A few questions, then I will tell you where we fit.';
    }
    function progress(n) { if (fill) fill.style.width = Math.min(100, (n / 6) * 100) + '%'; }
    function esc(t) { return String(t).replace(/[<>&]/g, function (c) {
      return { '<': '&lt;', '>': '&gt;', '&': '&amp;' }[c]; }); }

    function ask() {
      progress(history.length);
      if (!key) { collectWords(); return; }
      var q = Q[key];
      var html = '<div class="bda-card"><p class="bda-q">' + q.q + '</p><div class="bda-opts">';
      q.a.forEach(function (o, n) {
        html += '<button class="bda-opt" type="button" data-i="' + n + '">' + o.t + '</button>';
      });
      html += '</div>';
      if (history.length) html += '<button class="bda-back" type="button" data-back>&larr; Back</button>';
      stage.innerHTML = html + '</div>';
      stage.querySelectorAll('.bda-opt').forEach(function (b) {
        b.addEventListener('click', function () {
          var o = Q[key].a[parseInt(b.getAttribute('data-i'), 10)];
          history.push({ key: key, caps: caps.slice(), notes: notes.slice(), tag: tag });
          if (o.caps) caps = caps.concat(o.caps);
          if (o.tag) tag = o.tag;
          notes.push(o.t);
          key = o.next;
          ask();
    ...
    function stepBack() {
      ...
      if (!prev) return;
      key = prev.key; caps = prev.caps; notes = prev.notes; tag = prev.tag;
      ask();
    }

    function collectWords() {
      progress(4);
      stage.innerHTML = '<div class="bda-card"><p class="bda-q">In your own words, what is happening?</p>' +
        '<textarea class="bda-area" data-words rows="4" placeholder="A few lines is plenty. Where it hurts, and what you have already tried."></textarea>' +
        '<p class="bda-err" data-err hidden></p>' +
        '<button class="bda-next" type="button" data-go>Continue</button>' +
        '<button class="bda-back" type="button" data-back>&larr; Back</button></div>';
      var ta = stage.querySelector('[data-words]');
      ta.value = words;
      stage.querySelector('[data-go]').addEventListener('click', function () {
        words = ta.value.trim();
        if (words.length < 8) {
          var e = stage.querySelector('[data-err]');
          e.textContent = 'A sentence or two, so we know what we are looking at.';
          e.hidden = false;
          return;
        }
        collectDetails();
      });
      stage.querySelector('[data-back]').addEventListener('click', function () { key = 'when'; stepBack(); });
    }

    function collectDetails() {
      progress(5);
      stage.innerHTML = '<div class="bda-card"><p class="bda-q">Who should we come back to?</p>' +
        '<input class="bda-input" data-n placeholder="Your name" autocomplete="name"/>' +
        '<input class="bda-input" data-e type="email" placeholder="Work email" autocomplete="email"/>' +
        '<input class="bda-input" data-c placeholder="Company (optional)" autocomplete="organization"/>' +
        '<p class="bda-err" data-err hidden></p>' +
        '<button class="bda-next" type="button" data-go>See what we would suggest</button>' +
        '<button class="bda-back" type="button" data-back>&larr; Back</button></div>';
      var n = stage.querySelector('[data-n]'), e = stage.querySelector('[data-e]'), c = stage.querySelector('[data-c]');
      n.value = det.name; e.value = det.email; c.value = det.company;
      stage.querySelector('[data-go]').addEventListener('click', function () {
        det = { name: n.value.trim(), email: e.value.trim(), company: c.value.trim() };
        var ok = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(det.email);
        if (!det.name || !ok) {
          var err = stage.querySelector('[data-err]');
          err.textContent = det.name ? 'That email does not look right.' : 'We need a name and an email to reply to.';
          err.hidden = false;
          return;
        }
        summarise();
      });
      stage.querySelector('[data-back]').addEventListener('click', collectWords);
    }

    function summarise() {
      progress(6);
      if (sub) sub.textContent = 'Here is what I have noted.';
      ...
      stage.innerHTML = '<div class="bda-card">' +
        '<div class="bda-note"><span class="bda-note-k">Noted</span><p>' +
        (tag ? '<b>' + esc(tag) + '</b> &middot; ' : '') + esc(notes.slice(-3).join(' · ')) + '</p>' +
        '<p class="bda-note-w">&ldquo;' + esc(words) + '&rdquo;</p></div>' +
        '<div class="bda-caps"><span class="bda-note-k">Where we would start</span><ul>' +
        lines.slice(0, 4).map(function (l) { return '<li>' + l + '</li>'; }).join('') + '</ul></div>' +
        '<p class="bda-ask">Worth half an hour with someone who has run this? Send it over and we will come back with times.</p>' +
        '<button class="bda-next" type="button" data-send>Send this and book a call</button>' +
        '<button class="bda-back" type="button" data-back>&larr; Change something</button></div>';
      stage.querySelector('[data-send]').addEventListener('click', send);
      stage.querySelector('[data-back]').addEventListener('click', collectDetails);
    }
    ...
    function send(ev) {
      ...
      var problem = 'Area: ' + (tag || 'unspecified') + '\nAnswers: ' + notes.join(' | ') +
                    '\n\nIn their words:\n' + words;
      function done(msg, fallback) {
        if (sub) sub.textContent = 'Thanks.';
        stage.innerHTML = '<div class="bda-card"><p class="bda-done">' + msg + '</p>' +
          (fallback
            ? '<div class="contactpop-rows"><div class="contactpop-row"><span class="contactpop-label">Email</span>' +
              '<a class="contactpop-val" href="mailto:' + INBOX + '?subject=' +
              encodeURIComponent('Problem worth solving - ' + det.name) + '&body=' +
              encodeURIComponent(problem) + '">' + INBOX + '</a></div>' +
              '<div class="contactpop-row"><span class="contactpop-label">Phone</span>' +
              '<a class="contactpop-val" href="tel:+919497153297">+91 94971 53297</a></div></div>'
            : (BOOKING_URL ? '<a class="bda-next" href="' + BOOKING_URL + '" target="_blank" rel="noopener">Pick a time now &rarr;</a>' : '')) +
          '</div>';
      }
      fetch(ENDPOINT, {
        method: 'POST',
        ...
        done('That has come through. We will reply to ' + esc(det.email) + ' with a couple of times.', false);
      }).catch(function () {
        /* Never swallow it: hand back everything they typed. */
        done('We could not send that automatically. Mail or call us and we will pick it up - your answers are pre-filled in the link below.', true);
      });
    }
    ...
    function open() {
      ...
      var c = pop.querySelector('[data-contactpop-close]');
      if (c) c.focus();
    }
    function close() {
