...
(function () {
  ...
  /* Nuggets: SCM facts on a sliding track.
     One slide at a time, translated rather than cross-faded — the
     horizontal movement is what tells you more exists.

     Pauses on hover, on focus, and while the tab is hidden: a timer left
     running in a background tab means returning to a slide that changed
     without you, with a progress bar that lied about when the next was
     due. */
  (function () {
    var box = document.querySelector('[data-nugbox]');
    if (!box) return;
    var track = box.querySelector('[data-nug-track]');
    var slides = [].slice.call(box.querySelectorAll('.nug'));
    ...
    var prev = box.querySelector('[data-nug-prev]');
    var next = box.querySelector('[data-nug-next]');
    if (!track || slides.length < 2) return;

    var HOLD = 10000;
    var i = 0, t0 = 0, elapsed = 0, paused = false, held = false;
    var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    function show(n, resetTimer) {
    ...
    function tick(now) {
      ...
      if (paused || held) { t0 = now - elapsed; return; }
      elapsed = now - t0;
      if (fill) fill.style.width = Math.min(100, (elapsed / HOLD) * 100) + '%';
      if (elapsed >= HOLD) show(i + 1);
    }

    if (prev) prev.addEventListener('click', function () { show(i - 1); });
    if (next) next.addEventListener('click', function () { show(i + 1); });
    dots.forEach(function (d) {
      d.addEventListener('click', function () { show(parseInt(d.getAttribute('data-nug-go'), 10) || 0); });
    });
    if (pauseBtn) {
    ...
    show(0);
    if (!reduce) { t0 = performance.now(); requestAnimationFrame(tick); }
