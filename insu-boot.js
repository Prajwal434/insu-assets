...
(function () {
  ...
  try {
    ...
    /* Document deck + overlay viewer. */
    (function () {
      var track = document.querySelector('[data-deck-track]');
      var viewer = document.querySelector('[data-pdfviewer]');
      if (!track || !viewer) return;

      /* The overlay is position:fixed, so it must not sit inside an
         ancestor with a transform — that would make the ancestor its
         containing block and trap it inside the panel. Same trap that
         hid the nav and the logo earlier. */
      if (viewer.parentNode !== document.body) document.body.appendChild(viewer);

      var frame = viewer.querySelector('[data-pdfviewer-frame]');
      var title = viewer.querySelector('[data-pdfviewer-title]');
      var openA = viewer.querySelector('[data-pdfviewer-open]');
      var dlA = viewer.querySelector('[data-pdfviewer-dl]');
      var fbA = viewer.querySelector('[data-pdfviewer-fb]');
      var lastFocus = null;
      ...
      function open(url, label) {
        ...
        var close = viewer.querySelector('[data-pdfviewer-close]');
      ...
      function close() {
        ...
        /* Drop the src so the PDF stops loading and the next open
           starts clean rather than showing the previous document. */
        if (frame) frame.removeAttribute('src');
        if (lastFocus && lastFocus.focus) lastFocus.focus();
      }

      document.querySelectorAll('.pdfcard').forEach(function (card) {
        card.addEventListener('click', function () {
          var tmp = document.createElement('textarea');
          tmp.innerHTML = card.getAttribute('data-title') || '';
          open(card.getAttribute('data-pdf'), tmp.value);
        });
      });

      var closeBtn = viewer.querySelector('[data-pdfviewer-close]');
      ...
      /* Arrows scroll by one card, measured rather than assumed so the
         responsive card width stays the single source of truth. */
      var prev = document.querySelector('[data-deck-prev]');
      var next = document.querySelector('[data-deck-next]');
      function step() {
        var card = track.querySelector('.pdfcard');
        if (!card) return 320;
