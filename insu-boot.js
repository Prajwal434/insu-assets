...
(function () {
  ...
  /* "One project, seven weeks" — the teaching differentiator, shown
     rather than claimed. Layers accumulate: each week adds to the same
     project and the earlier ones stay on screen, because the point is
     that nothing gets thrown away. */
  (function () {
    var box = document.querySelector('[data-grow]');
    if (!box) return;
    var steps = [].slice.call(box.querySelectorAll('.grow-step'));
    var layers = [].slice.call(box.querySelectorAll('.grow-layer'));
    var end = box.querySelector('[data-grow-end]');
    ...
    function show(n) {
      ...
      if (end) end.hidden = n < layers.length - 1;
    }
