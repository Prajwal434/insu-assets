...
(function () {
  ...
  /* Insu: a scripted first-line consultant, played as a conversation.

     Chat rather than a form — the agent types, reacts to what was just
     said, then asks the next thing. Quick replies for the branching
     questions, free text where a person would actually write.

     It is a decision tree and the panel says so. Nothing here calls a
     model; the honesty costs nothing and the alternative is a claim the
     first curious visitor disproves. */
  (function () {
    var ENDPOINT = '/_functions/contactRequest';
    var INBOX = 'info@insudynamics.com';
    var BOOKING_URL = '';   /* Wix Bookings or Calendly, once one exists */
    ...
    var log = pop.querySelector('[data-bda-log]');
    var replyBox = pop.querySelector('[data-bda-reply]');
    var chips = pop.querySelector('[data-bda-chips]');
    var form = pop.querySelector('[data-bda-form]');
    var input = pop.querySelector('[data-bda-input]');
    var sub = pop.querySelector('[data-bda-sub]');
    if (!log || !chips) return;
    ...
    var CAPS = {
      planning:  ['Plan setup, sourcing rules and constraint modelling on ASCP or Fusion', 'Exception design, so planners read what matters instead of thousands', 'Planner enablement, so the plan can be defended not just read'],
      inventory: ['Demand variability analysis, then ABC and XYZ together', 'Safety stock and reorder policy per segment, service-level driven', 'Slow and obsolete stock surfaced with what it costs to hold'],
      forecast:  ['Statistical baselines with business override on top', 'Bias and MAPE tracked separately across horizons', 'Consensus planning across sales, ops and finance'],
      agents:    ['An agent answering in plain language over your own data', 'Every figure returned with the record behind it', 'Exceptions ranked by consequence, not by age'],
      infra:     ['Monitoring that acts on the slope, before the breach', 'First-level remediation without paging anyone', 'Each incident captured as a rule it plans against'],
      data:      ['Pipelines out of the systems the data is stranded in', 'Measures people act on rather than another dashboard', 'One version of a number, with its source'],
    ...
    var FLOW = {
      area: {
        say: ['So I know where to point this — what are you bringing us?'],
        chips: [
          { t: 'Supply chain planning', tag: 'Supply chain', next: 'sc',
            re: 'Our home ground, so you will get a straight answer.' },
          { t: 'Something to automate with AI', tag: 'AI and automation', next: 'ai',
            re: 'Good. Though sometimes the honest answer is that AI is not the fix.' },
          { t: 'Data we cannot decide from', tag: 'Data and reporting', caps: ['data'], next: 'cost',
            re: 'Usually not a data problem so much as a trust problem.' },
          { t: 'Not sure, something is wrong', tag: 'Discovery', caps: ['discover'], next: 'cost',
            re: 'That is a fine place to start. Most useful work begins there.' }
        ]
      },
      sc: {
        say: ['Where does it hurt most?'],
        chips: [
          { t: 'The plan is not trusted', caps: ['planning'], next: 'cost',
            re: 'Then people are back on spreadsheets. The software is rarely the cause.' },
          { t: 'Too much stock, still stocking out', caps: ['inventory'], next: 'cost',
            re: 'Usually a policy problem wearing a forecasting costume.' },
          { t: 'The forecast is always wrong', caps: ['forecast'], next: 'cost',
            re: 'Worth separating accuracy from bias. They need different fixes.' },
          { t: 'Oracle is live but unused', caps: ['planning'], next: 'cost',
            re: 'Seen it often. The setup usually stopped describing the business.' }
        ]
      },
      ai: {
        say: ['What would you point it at?'],
        chips: [
          { t: 'Questions asked every week', caps: ['agents'], next: 'cost',
            re: 'Those are the ones worth automating first.' },
          { t: 'Infrastructure and incidents', caps: ['infra'], next: 'cost',
            re: 'That is close to what SRE Bench already does.' },
          { t: 'Documents and manual entry', caps: ['data','agents'], next: 'cost',
            re: 'Often the fastest payback of the lot.' },
          { t: 'Predicting things earlier', caps: ['forecast','agents'], next: 'cost',
            re: 'Then the question is what you would do differently with the warning.' }
        ]
      },
      cost: {
        say: ['And what is it costing you today?'],
        chips: [
          { t: "People's time, every week", next: 'when', re: 'That compounds quietly.' },
          { t: 'Money — stock, expedites, overtime', next: 'when', re: 'Then it should be measurable, which helps.' },
          { t: 'Late or guessed decisions', next: 'when', re: 'The expensive kind, and the hardest to put a number on.' },
          { t: 'Hard to quantify yet', next: 'when', re: 'Fine. Working that out is often the first job.' }
        ]
      },
      when: {
        say: ['How soon does it need to move?'],
        chips: [
          { t: 'Urgent', next: 'words', re: 'Understood.' },
          { t: 'This quarter', next: 'words', re: 'Sensible timeframe.' },
          { t: 'Planning ahead', next: 'words', re: 'Good — the early conversations are the useful ones.' }
        ]
      },
      words: {
        say: ['Tell me in your own words what is happening. A couple of lines is plenty.'],
        ask: 'words', next: 'name'
      },
      name: { say: ['Got it. And your name?'], ask: 'name', next: 'email' },
      email: { say: ['Thanks {name}. Best email to come back on?'], ask: 'email', next: 'company' },
      company: { say: ['And the company? Say skip if you would rather not.'], ask: 'company', next: 'summary' }
    };

    var step, caps, notes, tag, words, det, busy;
    function reset() {
      clearAll();
      step = 'area'; caps = []; notes = []; tag = '';
      words = ''; det = { name: '', email: '', company: '' }; busy = false;
      log.innerHTML = '';
      chips.innerHTML = '';
    ...
    function bubbleUser(text) {
      ...
      d.className = 'bda-msg bda-msg--me';
      d.innerHTML = '<span>' + esc(text) + '</span>';
      log.appendChild(d); scroll();
    }

    /* Types one agent line, then calls back. A typing indicator first,
       because a reply appearing instantly reads as a form, not a reply. */
    function bubbleAgent(text, done) {
      var wrap = document.createElement('div');
      wrap.className = 'bda-msg bda-msg--them';
      wrap.innerHTML = '<span class="bda-typing"><i></i><i></i><i></i></span>';
      log.appendChild(wrap); scroll();
      if (sub) sub.textContent = 'typing…';

      var think = Math.min(900, 260 + text.length * 9);
      later(function () {
        wrap.innerHTML = '<span></span>';
        var span = wrap.firstChild;
        if (reduce) { span.textContent = text; if (sub) sub.textContent = 'online'; scroll(); if (done) done(); return; }
        var i = 0;
    ...
    /* Say a list of lines in order, then continue. */
    function say(lines, done) {
      var i = 0;
      (function next() {
        if (i >= lines.length) { if (done) done(); return; }
        bubbleAgent(lines[i++], function () { later(next, 220); });
      })();
    }
    ...
    function showChips(list) {
      ...
      list.forEach(function (c, n) {
        ...
        b.addEventListener('click', function () {
          ...
          bubbleUser(c.t);
          notes.push(c.t);
          if (c.caps) caps = caps.concat(c.caps);
          if (c.tag) tag = c.tag;
          var lines = c.re ? [c.re] : [];
          say(lines, function () { busy = false; go(c.next); });
        });
        chips.appendChild(b);
      });
    ...
    function askText(kind, next) {
      chips.innerHTML = '';
      if (replyBox) replyBox.hidden = false;
      if (form) { form.hidden = false; }
      if (input) {
        input.value = '';
        input.placeholder = kind === 'words' ? 'What is happening…'
                          : kind === 'name' ? 'Your name'
                          : kind === 'email' ? 'you@company.com' : 'Company (or skip)';
        input.focus();
      }
      form.onsubmit = function (e) {
        e.preventDefault();
        var v = (input.value || '').trim();
        if (kind === 'words' && v.length < 8) return;
        if (kind === 'name' && !v) return;
        if (kind === 'email' && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(v)) {
          bubbleUser(v || '(blank)');
          say(['That does not look like an email — try again?'], function () { askText('email', next); });
          input.value = '';
          return;
        }
        bubbleUser(v);
        if (kind === 'words') words = v;
        if (kind === 'name') det.name = v;
        if (kind === 'email') det.email = v;
        if (kind === 'company') det.company = /^skip$/i.test(v) ? '' : v;
        form.hidden = true;
        go(next);
      };
    }
    ...
    function go(key) {
      if (!key) return;
      if (key === 'summary') { summarise(); return; }
      step = key;
      var node = FLOW[key];
      if (!node) return;
      var lines = node.say.map(function (l) { return l.replace('{name}', det.name || 'there'); });
      say(lines, function () {
        if (node.chips) showChips(node.chips);
        else askText(node.ask, node.next);
      });
    }

    function summarise() {
      var uniq = caps.filter(function (v, i) { return caps.indexOf(v) === i; });
      if (!uniq.length) uniq = ['discover'];
      var lines = [];
      uniq.slice(0, 2).forEach(function (k) { lines = lines.concat(CAPS[k] || []); });
      lines = lines.slice(0, 3);

      say(['Right — here is what I have.'], function () {
        var card = document.createElement('div');
        card.className = 'bda-msg bda-msg--them bda-summary';
        card.innerHTML =
          '<div class="bda-sum">' +
          '<span class="bda-note-k">Noted</span>' +
          '<p>' + (tag ? '<b>' + esc(tag) + '</b> · ' : '') + esc(notes.slice(-3).join(' · ')) + '</p>' +
          '<p class="bda-note-w">“' + esc(words) + '”</p>' +
          '<span class="bda-note-k">Where we would start</span><ul>' +
          lines.map(function (l) { return '<li>' + l + '</li>'; }).join('') + '</ul></div>';
        log.appendChild(card); scroll();
        say(['That is first-level only — a call would sharpen it. Shall I send this over and come back with times?'], function () {
          showChips([{ t: 'Yes, send it', next: '__send' }, { t: 'Not yet', next: '__no' }]);
          chips.querySelectorAll('.bda-chip').forEach(function (b, i) {
            b.onclick = function () {
              chips.innerHTML = '';
              bubbleUser(b.textContent);
              if (i === 0) send(); else say(['No problem. We are on ' + INBOX + ' whenever you want to pick it up.']);
            };
          });
        });
      });
    ...
    function send() {
      var problem = 'Area: ' + (tag || 'unspecified') + '\nAnswers: ' + notes.join(' | ') +
                    '\n\nIn their words:\n' + words;
      say(['Sending…'], function () {
        fetch(ENDPOINT, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: det.name, email: det.email,
                                 company: det.company || '(not given)', problem: problem })
        }).then(function (r) {
          if (!r.ok) throw new Error(r.status);
          say(['That is with us. We will come back to ' + det.email + ' with a couple of times.'], function () {
            if (BOOKING_URL) {
              var a = document.createElement('a');
              a.className = 'bda-booklink'; a.href = BOOKING_URL;
              a.target = '_blank'; a.rel = 'noopener';
              a.textContent = 'Or pick a time now →';
              log.appendChild(a); scroll();
            }
          });
        }).catch(function () {
          /* Never lose what they typed. */
          say(['I could not send that automatically. Mail it across and we will pick it up.'], function () {
            var a = document.createElement('a');
            a.className = 'bda-booklink';
            a.href = 'mailto:' + INBOX + '?subject=' +
                     encodeURIComponent('Problem worth solving - ' + det.name) +
                     '&body=' + encodeURIComponent(problem);
    ...
    function open() {
      ...
      later(function () {
        say(['Hi — I am Insu, first line here.',
             'I will ask a few things, then tell you honestly whether this is something we can help with.'],
            function () { go('area'); });
      }, 340);
    }
    function close() {
    ...
    var cb = pop.querySelector('[data-contactpop-close]');
  ...
  /* Back to top. Shows once the page has actually been travelled —
     a control that is always there is just clutter on a short scroll. */
  (function () {
    var btn = document.querySelector('[data-totop]');
    if (!btn) return;
    /* Fixed, so it must sit on <body> and not under a transformed
       ancestor — the trap that caught the nav and the logo. */
    if (btn.parentNode !== document.body) document.body.appendChild(btn);

    var shown = false, queued = false;
    function frame() {
      queued = false;
      var past = window.scrollY > window.innerHeight * 1.4;
      if (past === shown) return;
      shown = past;
      if (past) { btn.hidden = false; requestAnimationFrame(function () { btn.classList.add('is-on'); }); }
      else { btn.classList.remove('is-on'); setTimeout(function () { if (!shown) btn.hidden = true; }, 320); }
    }
  ...
  /* Capabilities grid: the button used to sit under all eight, so it
     promised more of something already fully shown. Four are visible
     and it reveals the rest. */
  (function () {
    var btn = document.querySelector('[data-solmore]');
    if (!btn) return;
    var rest = [].slice.call(document.querySelectorAll('.sol--more'));
    if (!rest.length) { btn.hidden = true; return; }

    btn.addEventListener('click', function () {
      var opening = rest[0].hidden;
      ...
      btn.innerHTML = opening ? 'Show fewer &uarr;' : 'Show all 8 capabilities &rarr;';
    });
  })();

  /* "One project, start to finish" — the teaching differentiator, shown
     rather than claimed.

     One set of stages per programme, because they genuinely differ: an
     analyst does not do deep learning and an AI engineer does not do
     Oracle SCM. Durations differ too, so the framing is stages rather
     than weeks — the header names the day ranges instead.

     Layers accumulate and earlier ones stay, because nothing being
     thrown away is the whole point. */
  (function () {
    var box = document.querySelector('[data-grow]');
    if (!box) return;
    var tabsEl = box.querySelector('[data-grow-tabs]');
    var stepsEl = box.querySelector('[data-grow-steps]');
    var layersEl = box.querySelector('[data-grow-layers]');
    var stackK = box.querySelector('[data-grow-stack-k]');
    var endEl = box.querySelector('[data-grow-end]');
    if (!tabsEl || !stepsEl || !layersEl) return;

    var PROGS = [
      { id: 'aieng', name: 'AI Engineering', file: 'late_orders.py',
        end: 'At the end that is not a certificate. It is something you can open in an interview.',
        steps: [
          ['Python', 'Reads a messy export of orders and says which are late.', 'late_orders.py', 'a script that answers one question'],
          ['Machine learning', 'Predicts which will slip — and measures whether it was right.', '+ a model, and its score', 'you can say how good it is'],
          ['Deep learning', 'Handles the patterns a simple model keeps missing.', '+ a stronger model', 'and the evidence it earns its cost'],
          ['Generative AI', 'Explains why, in plain language.', '+ explanation', 'it answers why, not just what'],
          ['LLMs and RAG', 'Grounds every answer in your own records.', '+ retrieval & citations', 'every figure carries its source'],
          ['Applications', 'Becomes something a colleague can use. Not a notebook.', '+ an interface', 'someone else can use it'],
          ['Deployment', 'Runs on a schedule, monitored, without you.', '+ scheduling & monitoring', 'it runs without you']
        ]},
      { id: 'analyst', name: 'Data Analyst', file: 'service_levels.sql',
        end: 'At the end you have a dashboard someone actually opens on a Monday.',
        steps: [
          ['Data', 'Gets a real, messy operational extract into shape.', 'service_levels.sql', 'the raw question, written down'],
          ['SQL', 'Answers it properly — joins, grain, the awkward edge cases.', '+ a query that holds up', 'right answer, repeatably'],
          ['Python', 'Automates the pull so nobody does it by hand again.', '+ automation', 'it refreshes itself'],
          ['Analytics', 'Segments it, so the number stops being one number.', '+ segmentation', 'you can see where it goes wrong'],
          ['Visualization', 'Makes it readable by someone who was not there.', '+ charts that explain', 'it survives being forwarded'],
          ['Insights', 'Ends on a recommendation, not a screenshot.', '+ a recommendation', 'someone can act on it']
        ]},
      { id: 'science', name: 'Data Science', file: 'demand_forecast.ipynb',
        end: 'At the end you can defend the model — which is the part interviews actually test.',
        steps: [
          ['Data', 'Real demand history, with all the gaps that implies.', 'demand_forecast.ipynb', 'the problem, framed'],
          ['Statistics', 'Establishes what the data can and cannot support.', '+ a baseline', 'a number to beat'],
          ['Machine learning', 'Beats the baseline, or explains why it did not.', '+ a model', 'and an honest comparison'],
          ['Models', 'Tries the alternatives properly rather than the first that works.', '+ candidates compared', 'chosen on evidence'],
          ['Evaluation', 'Measures accuracy and bias separately. They differ.', '+ error analysis', 'you know how it fails'],
          ['Deployment', 'Gets it out of the notebook and into something callable.', '+ a served model', 'it exists outside your laptop']
        ]},
      { id: 'scm', name: 'Supply Chain Planning', file: 'plan_review.xlsx',
        end: 'At the end you have run a planning cycle end to end, which is what the job actually is.',
        steps: [
          ['Fundamentals', 'Maps how the operation really plans today.', 'plan_review.xlsx', 'the current policy, written down'],
          ['Demand', 'Builds a forecast, and measures its bias.', '+ a demand plan', 'and how wrong it usually is'],
          ['Supply', 'Turns it into something the network can actually make.', '+ a supply plan', 'constrained, not wished for'],
          ['Inventory', 'Sets safety stock per segment instead of one blanket rule.', '+ an inventory policy', 'service level by segment'],
          ['Oracle SCM', 'Runs it in the system planners really use.', '+ a configured plan', 'in ASCP or Fusion, not a spreadsheet'],
          ['Analytics', 'Reads the exceptions that matter and ignores the rest.', '+ exception triage', 'ranked by consequence']
        ]}
    ];

    var cur = 0, at = -1, timer = null, held = false;
    var reduce = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    tabsEl.innerHTML = PROGS.map(function (p, i) {
      return '<button class="grow-tab' + (i === 0 ? ' is-on' : '') + '" type="button" data-p="' + i + '">' +
             p.name + '</button>';
    }).join('');

    function render(pi) {
      var prog = PROGS[pi];
      stepsEl.innerHTML = prog.steps.map(function (st, n) {
        return '<li class="grow-step" data-n="' + n + '"><span class="grow-num">' +
               ('0' + (n + 1)).slice(-2) + '</span><span class="grow-t"><b>' + st[0] +
               '</b><span>' + st[1] + '</span></span></li>';
      }).join('');
      layersEl.innerHTML = prog.steps.map(function (st, n) {
        return '<div class="grow-layer" data-l="' + n + '"><b>' + st[2] + '</b><span>' + st[3] + '</span></div>';
      }).join('');
      if (stackK) stackK.textContent = 'Your project · ' + prog.steps.length + ' stages';
      if (endEl) { endEl.textContent = prog.end; endEl.hidden = true; }
      [].slice.call(stepsEl.children).forEach(function (el, n) {
        el.addEventListener('click', function () { stop(); show(n); });
      });
      at = -1;
      show(0);
    }
    ...
    function show(n) {
      at = n;
      var steps = [].slice.call(stepsEl.children);
      var layers = [].slice.call(layersEl.children);
      steps.forEach(function (el, k) {
        el.classList.toggle('is-on', k === n);
        el.classList.toggle('is-done', k < n);
      });
      layers.forEach(function (el, k) { el.classList.toggle('is-on', k <= n); });
      if (endEl) endEl.hidden = n < layers.length - 1;
    }
    ...
    function play() {
      ...
      timer = setInterval(function () {
        if (held) return;
        if (at >= PROGS[cur].steps.length - 1) { stop(); return; }
        show(at + 1);
      }, 1500);
    }

    tabsEl.querySelectorAll('.grow-tab').forEach(function (b) {
      b.addEventListener('click', function () {
        cur = parseInt(b.getAttribute('data-p'), 10);
        tabsEl.querySelectorAll('.grow-tab').forEach(function (o) {
    ...
    /* Reading it is the point; do not advance under someone's cursor. */
    box.addEventListener('mouseenter', function () { held = true; });
    box.addEventListener('mouseleave', function () { held = false; });

    render(0);
    if (reduce) { show(PROGS[0].steps.length - 1); return; }

    var panel = box.closest('.pageview');
    var scroller = panel && panel.querySelector('.pageview-scroll');
    if ('IntersectionObserver' in window) {
      var io = new IntersectionObserver(function (es) {
        es.forEach(function (e) { if (e.isIntersecting) { io.unobserve(e.target); play(); } });
      }, scroller ? { root: scroller, threshold: 0.3 } : { threshold: 0.3 });
      io.observe(box);
    } else { play(); }
  })();
    ...
    (function () {
      ...
      (function intro() {
        ...
        function play() {
          ...
          var kwb = panel.querySelector('[data-aiintro-kw]');
          if (kwb) { kwb.hidden = true; kwb.classList.remove('is-out'); }
          el.hidden = false;
          el.setAttribute('aria-hidden', 'false');
          document.body.classList.add('intro-playing');
          var i = 0;
          function step() {
            if (i >= LINES.length) { later(keywords, 380); return; }
        ...
        /* A two-second beat naming what we work with, before the walkthrough.

           Marketing vocabulary belongs here rather than inside the build

           steps, where naming the technique would dilute the specifics. */

        var KEYWORDS = ['LLMs', 'RAG', 'Agentic AI', 'On-premise', 'On-device', 'Cloud', 'Fully offline'];

        function keywords() {

          var wrap = panel.querySelector('[data-aiintro-kw]');

          var row = panel.querySelector('[data-aiintro-kw-row]');

          var inner = panel.querySelector('.aiintro-inner');

          if (!wrap || !row) { flow(); return; }

          if (inner) inner.hidden = true;

          row.innerHTML = KEYWORDS.map(function (k) {

            return '<span class="aiintro-kw-i">' + k + '</span>';
          ...
          items.forEach(function (el, n) {
            ...
            later(function () { el.classList.add('is-on'); }, n * 210);
