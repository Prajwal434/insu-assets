...
(function () {
  ...
  try {
    ...
    /* Case-study slide viewer. Arrow keys work when the viewer has focus,
       and the "open full size" link tracks the current page so it always
       opens what you are looking at rather than page one. */
    document.querySelectorAll('[data-slides]').forEach(function (wrap) {
      var slides = [].slice.call(wrap.querySelectorAll('.sc-slide'));
      if (slides.length < 2) return;
      var dots = [].slice.call(wrap.querySelectorAll('.sc-dot'));
      var prev = wrap.querySelector('[data-prev]');
      var next = wrap.querySelector('[data-next]');
      var counter = wrap.querySelector('[data-current]');
      var full = wrap.querySelector('.sc-slide-full');
      var i = 0;
      ...
      function show(n) {
        ...
        if (full) full.href = slides[i].getAttribute('src');
      }

      if (prev) prev.addEventListener('click', function () { show(i - 1); });
      if (next) next.addEventListener('click', function () { show(i + 1); });
      dots.forEach(function (d) {
        d.addEventListener('click', function () { show(parseInt(d.getAttribute('data-go'), 10) || 0); });
      });
