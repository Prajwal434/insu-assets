...
(function () {
  ...
  try {
    ...
    (function () {
      ...
      if (!root) return;
      var greet = root.querySelector('[data-pf-greet]');
      var gLine = root.querySelector('[data-pf-greet-line]');
      ...
      var GREETING = [
        'Hi. I can help you pick a path.',
        'A few quick questions — nothing you need to prepare for.'
      ];
      ...
      /* Type the greeting, then hand over. Skipped entirely under reduced
         motion — a typing effect is decoration and the questions are the
         point. */
      if (reduce || !gLine || !greet) { start(); return; }
      var gi = 0;
      function typeLine() {
        if (gi >= GREETING.length) { later(start, 520); return; }
        var text = GREETING[gi], c = 0;
        gLine.textContent = '';
        var typer = setInterval(function () {
          gLine.textContent = text.slice(0, ++c);
          if (c >= text.length) {
            clearInterval(typer);
            later(function () {
              gLine.style.opacity = '0';
              later(function () { gLine.style.opacity = '1'; gi++; typeLine(); }, 260);
            }, 900);
          }
        }, 26);
        timers.push(typer);
      }
      gLine.style.transition = 'opacity .26s ease';

      /* Only greet once it is actually on screen. */
      if ('IntersectionObserver' in window) {
        var io = new IntersectionObserver(function (es) {
          es.forEach(function (e) { if (e.isIntersecting) { io.unobserve(e.target); typeLine(); } });
        }, { threshold: 0.35 });
        io.observe(root);
      } else { typeLine(); }
    })();
