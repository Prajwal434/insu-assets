...
(function () {
  ...
  try {
    ...
    (function () {
      ...
      (function intro() {
        ...
        function flow() {
          ...
          var list = panel.querySelector('[data-aiintro-steps]');

          if (!wrap || !list) { end(); return; }

          list.innerHTML = STEPS.map(function (st, n) {

            return '<li class="aiintro-step"><span class="aiintro-step-n">' + (n + 1) + '</span>' +

                   '<span class="aiintro-step-t"><b>' + st[0] + '</b><span>' + st[1] + '</span></span></li>';
          ...
          items.forEach(function (el, n) {
            ...
            later(function () {
              ...
              items.forEach(function (o, k) {
                ...
                o.classList.toggle('is-past', k < n);
          ...
          later(function () {
            ...
            items.forEach(function (o) { o.classList.remove('is-on'); o.classList.add('is-past'); });

          }, items.length * 1000);

          later(end, items.length * 1000 + 900);
      ...
      (function pegging() {
        ...
        var tree = host.querySelector('[data-peg-tree]');
        var note = host.querySelector('[data-peg-note]');
        var replay = host.querySelector('[data-peg-replay]');
        if (!typed || !tree) return;

        var QUERY = 'trace SO-48213 — why is it late?';
        var NODES = [
          ...
          { d: 2, k: 'REQ',    id: 'PR-2203',   m: 'lead time 14 → 21 days', risk: true }
        ];
        ...
        function run() {
          ...
          var c = 0;
          var typer = setInterval(function () {
            typed.textContent = QUERY.slice(0, ++c);
            if (c >= QUERY.length) { clearInterval(typer); later(drawNodes, 420); }
          }, 32);
          timers.push(typer);
        }
        ...
        function drawNodes() {
          NODES.forEach(function (n, i) {
            later(function () {
              ...
              row.className = 'peg' + (n.risk ? ' is-risk' : '');
              row.setAttribute('data-depth', n.d);
              row.innerHTML = '<span class="peg-kind">' + n.k + '</span>' +
                              '<span class="peg-id">' + n.id + '</span>' +
                              '<span class="peg-meta">' + n.m + '</span>';
              tree.appendChild(row);
            }, i * 380);
          });
          later(function () {
            if (note) {
              note.innerHTML = 'Root cause: <b>PR-2203</b> — supplier lead time moved 14 to 21 days, ' +
                               'which pushes WO-4471 past the order date. Two options: pull 1,600 units ' +
                               'from the Pune DC, or split the order and ship 3,200 on time.';
              note.hidden = false;
            }
            if (replay) replay.hidden = false;
          }, NODES.length * 380 + 400);
        }

        if (replay) replay.addEventListener('click', run);

        /* Start when it is actually on screen, not on page load. */
    ...
    /* Pathfinder: five questions, then two suggestions — the path they
       ...
       Deliberately a scoring table rather than anything cleverer. Each
       answer adds weight to one or two programmes; the totals decide.
       It is described as a questionnaire on the page for the same
       reason: calling a decision tree "AI" on an AI company's site is a
       claim the first curious developer disproves. */
    (function () {
      ...
      var QUESTIONS = [
        { q: 'Where do you picture yourself?',
          kind: 'goal',
          a: [
            { t: 'Making sense of data for the business', pick: 'analyst' },
            { t: 'Building models that predict what happens next', pick: 'science' },
            { t: 'Shipping AI systems people actually use', pick: 'aieng' },
            { t: 'Running planning for a real operation', pick: 'scm' },
            { t: 'Honestly, not sure yet', pick: null }
          ] },
        { q: 'What is pulling you that way?',
          a: [
            { t: 'It is where the work is', w: { analyst: 1, scm: 1 } },
            { t: 'I want to understand how it actually works', w: { science: 3, aieng: 1 } },
            { t: 'I want to build things', w: { aieng: 2, python: 2 } },
            { t: 'I want my work to change a decision', w: { analyst: 2, scm: 2 } },
            { t: 'I am changing career and need a foothold', w: { python: 3, analyst: 1 } }
          ] },
        { q: 'What are you genuinely good at?',
          a: [
            { t: 'Spotting patterns in numbers', w: { science: 3, analyst: 2 } },
            { t: 'Explaining complicated things simply', w: { analyst: 3, scm: 1 } },
            { t: 'Taking things apart and getting them working', w: { aieng: 3, python: 2 } },
            { t: 'Bringing order to a messy process', w: { scm: 3, analyst: 1 } },
            { t: 'Making things clear to look at', w: { analyst: 2 } }
          ] },
        { q: 'Something breaks the day before a deadline. You...',
          a: [
            { t: 'Dig into the data to find out why', w: { science: 3, analyst: 1 } },
            { t: 'Go and ask the people involved', w: { scm: 3 } },
            { t: 'Rebuild the broken part yourself', w: { aieng: 3, python: 1 } },
            { t: 'Write down what went wrong so it cannot repeat', w: { scm: 2, analyst: 1 } }
          ] },
        { q: 'In a normal week, you would rather spend most of your time...',
          a: [
            { t: 'Analysing and explaining', w: { analyst: 2, science: 2 } },
            { t: 'Writing and debugging code', w: { aieng: 3, python: 2 } },
            { t: 'Coordinating people and plans', w: { scm: 3 } },
            { t: 'A bit of everything', w: { python: 3 } }
          ] }
      ];
      ...
      function setProgress() {
        if (fill) fill.style.width = (step / QUESTIONS.length) * 100 + '%';
      }

      function render() {
        setProgress();
        if (step >= QUESTIONS.length) { result(); return; }
        var q = QUESTIONS[step];
        var html = '<div class="pf-card">' +
          '<span class="pf-step">Question ' + (step + 1) + ' of ' + QUESTIONS.length + '</span>' +
          '<h3 class="pf-q">' + q.q + '</h3><div class="pf-opts">';
        q.a.forEach(function (opt, i) {
          html += '<button class="pf-opt" type="button" data-i="' + i + '">' +
                  '<span class="pf-opt-t">' + opt.t + '</span>' +
                  '<span class="pf-opt-go" aria-hidden="true">&rarr;</span></button>';
        });
        html += '</div>';
        if (step > 0) html += '<button class="pf-back" type="button" data-pf-back>&larr; Back</button>';
        html += '</div>';
        stage.innerHTML = html;

        stage.querySelectorAll('.pf-opt').forEach(function (b) {
          b.addEventListener('click', function () {
            var opt = q.a[parseInt(b.getAttribute('data-i'), 10)];
            if (q.kind === 'goal') picked = opt.pick;
            if (opt.w) Object.keys(opt.w).forEach(function (k) { scores[k] += opt.w[k]; });
            chosenText.push(opt.t);
            step++;
            render();
          });
        });
        var back = stage.querySelector('[data-pf-back]');
        if (back) back.addEventListener('click', restartFrom);
      }

      /* Going back has to undo the score too, or answers accumulate and
         the result drifts with every correction. Simplest correct thing:
         replay the remembered answers from the start. */
      function restartFrom() {
        var keep = chosenText.slice(0, step - 1);
        step = 0; picked = null; chosenText = [];
        Object.keys(scores).forEach(function (k) { scores[k] = 0; });
        keep.forEach(function (text) {
          var q = QUESTIONS[step];
          for (var i = 0; i < q.a.length; i++) {
            if (q.a[i].t === text) {
              if (q.kind === 'goal') picked = q.a[i].pick;
              if (q.a[i].w) Object.keys(q.a[i].w).forEach(function (k) { scores[k] += q.a[i].w[k]; });
      ...
      function card(key, label, why) {
        var pr = PROGRAMMES[key];
        return '<article class="pf-res">' +
          '<span class="pf-res-tag">' + label + '</span>' +
          '<h4>' + pr.name + '</h4>' +
          '<p class="pf-res-note">' + pr.note + '</p>' +
          '<p class="pf-res-why">' + why + '</p>' +
          '</article>';
      }

      function result() {
        var order = ranked();
        var suggested = order[0];
        if (picked && suggested === picked) suggested = order[1];

        var html = '<div class="pf-card pf-done">' +
          '<span class="pf-step">Your result</span>' +
          '<h3 class="pf-q">' + (picked
              ? 'Where you are aiming, and where your answers point.'
              : 'Based on your answers.') + '</h3>' +
          '<div class="pf-results">';

        if (picked) {
          html += card(picked, 'What you chose',
            'You said this is where you picture yourself, so it stays on the table regardless.');
        }
        html += card(suggested, picked ? 'What your answers point to' : 'Strongest fit',
          'Your strengths and the way you described working landed hardest here.');
        html += '</div>' +
          '<p class="pf-res-foot">' + (picked && picked !== suggested
            ? 'Two different answers is normal, and worth a conversation rather than a coin toss.'
            : 'Your goal and your strengths agree, which makes this a straightforward one.') + '</p>' +
          '<div class="pf-actions">' +
          '<button class="btn" type="button" data-route="programs">See the programmes &rarr;</button>' +
        ...
        /* The programmes button lives inside markup this code just wrote,
           so the page's own [data-route] handler never bound to it. */
        var go = stage.querySelector('[data-route]');
        if (go) go.addEventListener('click', function () { location.hash = 'programs'; });
      }
