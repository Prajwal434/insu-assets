...
(function () {
  ...
  try {
    ...
    /* Embedded documents. The iframe src is set on first open so a
       half-megabyte PDF is never fetched for a card nobody expands. */
    document.querySelectorAll('[data-doc]').forEach(function (doc) {
      var card = doc.closest('details');
      var frame = doc.querySelector('.sc-doc-embed');
      if (!card || !frame) return;
      card.addEventListener('toggle', function () {
        if (card.open && !frame.getAttribute('src')) {
          frame.setAttribute('src', doc.getAttribute('data-src'));
        }
      });
    });
