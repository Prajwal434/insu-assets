...
(function () {
  ...
  try {
    ...
    /* Resource library: any [data-pdf] opens the document overlay. */
    (function () {
      var viewer = document.querySelector('[data-pdfviewer]');
      var triggers = [].slice.call(document.querySelectorAll('[data-pdf]'));
      if (!triggers.length) return;

      var frame, title, openA, dlA, fbA, lastFocus = null;
      if (viewer) {
        /* position:fixed, so it must not sit under a transformed
           ancestor — that becomes its containing block and would trap
           the overlay inside the panel. */
        if (viewer.parentNode !== document.body) document.body.appendChild(viewer);
        frame = viewer.querySelector('[data-pdfviewer-frame]');
        title = viewer.querySelector('[data-pdfviewer-title]');
        openA = viewer.querySelector('[data-pdfviewer-open]');
        dlA   = viewer.querySelector('[data-pdfviewer-dl]');
        fbA   = viewer.querySelector('[data-pdfviewer-fb]');
      }
      ...
      function open(url, label) {
        if (!viewer) { window.open(url, '_blank', 'noopener'); return; }
        lastFocus = document.activeElement;
        ...
        var c = viewer.querySelector('[data-pdfviewer-close]');
        if (c) c.focus();
      }
      function close() {
        ...
        if (frame) frame.removeAttribute('src');   // stop the fetch; start clean next time
        if (lastFocus && lastFocus.focus) lastFocus.focus();
      }
      ...
      triggers.forEach(function (el) {
        el.addEventListener('click', function (e) {
          e.preventDefault();
          open(el.getAttribute('data-pdf'), decode(el.getAttribute('data-title')));
        });
      });

      if (viewer) {
        var cb = viewer.querySelector('[data-pdfviewer-close]');
